 ...
 * 2021
 *
 * Authors: Sam Leigh, Tyler Wu, Bennett Greenberg, Felipe Paucar-Velasquez, Yuri Gershtein
...
void L1TrackJetEmulationProducer::L2_cluster(vector<Ptr<L1TTTrackType>> L1TrkPtrs_, vector<int>ttrk_, vector<int>tdtrk_, vector<int> ttdtrk_, MaxZBin &mzb) {
  const int nz = zBins_;
  ...
  for (int zbin=0; zbin<zBins_-1; ++zbin) {
    ...
    for (unsigned int k=0; k<L1TrkPtrs_.size(); ++k) {
      int trkpt = L1TrkPtrs_[k]->momentum().perp();
      int trketa = L1TrkPtrs_[k]->momentum().eta();
      int trkphi = L1TrkPtrs_[k]->momentum().phi();
      int trkZ = L1TrkPtrs_[k]->z0();

      for (int i=0; i<phiBins_; ++i) {
        for (int j=0; j<etaBins_; ++j) {
          L2cluster[k] = epbins[i][j];
          if ((152*k <=trkZ && zmax>=152*(k+1)) &&
          ((2730*j <= trketa && 2730*(j+1) >= trketa)
          && 152*i <= trkphi && 152*(i+1) >= trkphi
          && (zBinCount_[k] != 2))) {
...
bool L1TrackJetEmulationProducer::trackQualityCuts(float trk_pt,int trk_nstub, float trk_chi2,float trk_bendchi2, float trk_d0) {
